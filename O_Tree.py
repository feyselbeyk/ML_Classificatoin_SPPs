...
os.makedirs("O/IMERG/graphics", exist_ok=True)
os.makedirs("O/IMERG/CSV", exist_ok=True)
os.makedirs("O/IMERG", exist_ok=True)
# Load dataset
file_path = 'O/O.csv'
...
#plot_correlation(data, ['GAUGE', 'CMORPH', 'GSMAP', 'IMERG'], 'O/correlation_matrix.png')
...
plot_conf_matrix(y_test_nn, y_pred_nn, "Neural Network", 'O/IMERG/graphics/conf_matrix_nn.png')

# Plot confusion matrices for other models
for name, model in models.items():
    y_pred = model.predict(X_test_nn_scaled)
    plot_conf_matrix(y_test_nn, y_pred, name, f'O/IMERG/graphics/conf_matrix_{name.replace(" ", "_").lower()}.png')
...
plt.tight_layout()
plt.savefig("O/IMERG/graphics/model_comparison_accuracy.png", dpi=300)
plt.close()

# Save results to CSV
results_df.to_csv('O/IMERG/CSV/model_comparison_results.csv')

print("Model comparison results saved to 'O/IMERG/CSV/model_comparison_results.csv'.")
print("All graphics saved in 'O/IMERG/graphics/' directory.")
...
predictions_df.to_csv('O/IMERG/CSV/nn_predictions_with_features.csv', index=False)

print("Neural Network predictions with features saved to 'O/IMERG/CSV/nn_predictions_with_features.csv'.")
...
plt.ylabel('Feature')
plt.savefig('O/IMERG/graphics/rf_feature_importance.png', dpi=300)
plt.close()