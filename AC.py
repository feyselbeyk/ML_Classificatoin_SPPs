...
os.makedirs("AC/IMERG/graphics", exist_ok=True)
os.makedirs("AC/IMERG/CSV", exist_ok=True)
os.makedirs("AC/IMERG", exist_ok=True)
# Load dataset
file_path = 'AC/AC.csv'
...
#plot_correlation(data, ['GAUGE', 'CMORPH', 'GSMAP', 'IMERG'], 'AC/correlation_matrix.png')
...
plot_conf_matrix(y_test_nn, y_pred_nn, "Neural Network", 'AC/IMERG/graphics/conf_matrix_nn.png')

# Plot confusion matrices for other models
for name, model in models.items():
    y_pred = model.predict(X_test_nn_scaled)
    plot_conf_matrix(y_test_nn, y_pred, name, f'AC/IMERG/graphics/conf_matrix_{name.replace(" ", "_").lower()}.png')
...
plt.tight_layout()
plt.savefig("AC/IMERG/graphics/model_comparison_accuracy.png", dpi=300)
plt.close()

# Save results to CSV
results_df.to_csv('AC/IMERG/CSV/model_comparison_results.csv')

print("Model comparison results saved to 'AC/IMERG/CSV/model_comparison_results.csv'.")
print("All graphics saved in 'AC/IMERG/graphics/' directory.")
...
predictions_df.to_csv('AC/IMERG/CSV/nn_predictions_with_features.csv', index=False)

print("Neural Network predictions with features saved to 'AC/IMERG/CSV/nn_predictions_with_features.csv'.")
...
plt.ylabel('Feature')
plt.savefig('AC/IMERG/graphics/rf_feature_importance.png', dpi=300)
plt.close()