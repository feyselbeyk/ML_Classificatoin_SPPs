...
os.makedirs("AB/IMERG/graphics", exist_ok=True)
os.makedirs("AB/IMERG/CSV", exist_ok=True)
os.makedirs("AB/IMERG", exist_ok=True)
# Load dataset
file_path = 'AB/AB.csv'
...
#plot_correlation(data, ['GAUGE', 'CMORPH', 'GSMAP', 'IMERG'], 'AB/correlation_matrix.png')
...
plot_conf_matrix(y_test_nn, y_pred_nn, "Neural Network", 'AB/IMERG/graphics/conf_matrix_nn.png')

# Plot confusion matrices for other models
for name, model in models.items():
    y_pred = model.predict(X_test_nn_scaled)
    plot_conf_matrix(y_test_nn, y_pred, name, f'AB/IMERG/graphics/conf_matrix_{name.replace(" ", "_").lower()}.png')
...
plt.tight_layout()
plt.savefig("AB/IMERG/graphics/model_comparison_accuracy.png", dpi=300)
plt.close()

# Save results to CSV
results_df.to_csv('AB/IMERG/CSV/model_comparison_results.csv')

print("Model comparison results saved to 'AB/IMERG/CSV/model_comparison_results.csv'.")
print("All graphics saved in 'AB/IMERG/graphics/' directory.")
...
predictions_df.to_csv('AB/IMERG/CSV/nn_predictions_with_features.csv', index=False)

print("Neural Network predictions with features saved to 'AB/IMERG/CSV/nn_predictions_with_features.csv'.")
...
plt.ylabel('Feature')
plt.savefig('AB/IMERG/graphics/rf_feature_importance.png', dpi=300)
plt.close()