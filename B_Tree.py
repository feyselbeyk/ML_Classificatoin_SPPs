...
os.makedirs("C/GAUGE/graphics", exist_ok=True)
os.makedirs("C/GAUGE/CSV", exist_ok=True)
os.makedirs("C/GAUGE", exist_ok=True)
# Load dataset
file_path = 'C/C.csv'
...
plot_correlation(data, ['GAUGE', 'CMORPH', 'GSMAP', 'IMERG'], 'C/correlation_matrix.png')
...
y_nn = (data['GAUGE'] > 0).astype(int)  # Binary target
...
plot_conf_matrix(y_test_nn, y_pred_nn, "Neural Network", 'C/GAUGE/graphics/conf_matrix_nn.png')

# Plot confusion matrices for other models
for name, model in models.items():
    y_pred = model.predict(X_test_nn_scaled)
    plot_conf_matrix(y_test_nn, y_pred, name, f'C/GAUGE/graphics/conf_matrix_{name.replace(" ", "_").lower()}.png')
...
plt.tight_layout()
plt.savefig("C/GAUGE/graphics/model_comparison_accuracy.png", dpi=300)
plt.close()

# Save results to CSV
results_df.to_csv('C/GAUGE/CSV/model_comparison_results.csv')

print("Model comparison results saved to 'C/GAUGE/CSV/model_comparison_results.csv'.")
print("All graphics saved in 'C/GAUGE/graphics/' directory.")
...
predictions_df.to_csv('C/GAUGE/CSV/nn_predictions_with_features.csv', index=False)

print("Neural Network predictions with features saved to 'C/GAUGE/CSV/nn_predictions_with_features.csv'.")
...
plt.ylabel('Feature')
plt.savefig('C/GAUGE/graphics/rf_feature_importance.png', dpi=300)
plt.close()