...
os.makedirs("K/IMERG/graphics", exist_ok=True)
os.makedirs("K/IMERG/CSV", exist_ok=True)
os.makedirs("K/IMERG", exist_ok=True)
# Load dataset
file_path = 'K/k.csv'
...
#plot_correlation(data, ['GAUGE', 'CMORPH', 'GSMAP', 'IMERG'], 'K/correlation_matrix.png')
...
plot_conf_matrix(y_test_nn, y_pred_nn, "Neural Network", 'K/IMERG/graphics/conf_matrix_nn.png')

# Plot confusion matrices for other models
for name, model in models.items():
    y_pred = model.predict(X_test_nn_scaled)
    plot_conf_matrix(y_test_nn, y_pred, name, f'K/IMERG/graphics/conf_matrix_{name.replace(" ", "_").lower()}.png')
...
plt.tight_layout()
plt.savefig("K/IMERG/graphics/model_comparison_accuracy.png", dpi=300)
plt.close()

# Save results to CSV
results_df.to_csv('K/IMERG/CSV/model_comparison_results.csv')

print("Model comparison results saved to 'K/IMERG/CSV/model_comparison_results.csv'.")
print("All graphics saved in 'K/IMERG/graphics/' directory.")
...
predictions_df.to_csv('K/IMERG/CSV/nn_predictions_with_features.csv', index=False)

print("Neural Network predictions with features saved to 'K/IMERG/CSV/nn_predictions_with_features.csv'.")
...
plt.ylabel('Feature')
plt.savefig('K/IMERG/graphics/rf_feature_importance.png', dpi=300)
plt.close()